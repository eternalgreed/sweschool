...
class SimpleTree<T> {
    ...
    public ArrayList<T> EvenTrees() {
        ...
        maxPartitionTree(Root, result);
        return result;

    }

    private void maxPartitionTree(SimpleTreeNode<T> node, List<T> result) {
        if (node.Children != null) {
            for (int i = 0; i < node.Children.size(); i++) {
                SimpleTreeNode<T> child = node.Children.get(i);
                if (countNodes(node.Children.get(i)) % 2 == 0) {
                    result.add(child.Parent.NodeValue);
                    result.add(child.NodeValue);
                }
                maxPartitionTree(child, result);
            }
        }
    }
